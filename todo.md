...
1. 校验 `server_id`
2. 校验 token
3. 更新 `last_heartbeat_at`
4. 更新最近上报内容
5. 将 `heartbeat_fail_count` 清零
6. 若节点原状态为 `DOWN` 或 `SUSPECT`，触发恢复判断
...
每台 client 配置唯一 token。
server 收到 heartbeat 后，按 `server_id` 找到对应 token hash 进行校验。
...
- `hb-server.service`：HTTP API 服务
- `hb-monitor.service`：后台探测/状态评估服务

也可以把 API 与调度写在一个进程中，但分开更清晰。
